...
CREATE TABLE TREATMENT (ID BIGINT NOT NULL, TTYPE VARCHAR(31), DIAGNOSIS VARCHAR(255), patient_id BIGINT, providerid VARCHAR(255), PRIMARY KEY (ID))
...
CREATE TABLE PROVIDER (PROVIDERID BIGINT NOT NULL, NPI VARCHAR(255), SPECIALIZATION VARCHAR(255), NAME VARCHAR(255), PRIMARY KEY (PROVIDERID))
ALTER TABLE TREATMENT ADD CONSTRAINT FK_TREATMENT_patient_id FOREIGN KEY (patient_id) REFERENCES PATIENT (ID)
ALTER TABLE TREATMENT ADD CONSTRAINT FK_TREATMENT_providerid FOREIGN KEY (providerid) REFERENCES PROVIDER (NPI)
